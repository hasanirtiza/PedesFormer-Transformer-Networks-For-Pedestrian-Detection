...
def run_detector_on_dataset(day='day', mode='val'):
    ...
    eval_imgs = glob.glob('datasets/pedestrian_datasets/EuroCity/ECP/{}/img/{}/*/*'.format(day, mode))
    ...
    for i in range(args.checkpoint_start, args.checkpoint_end):
       ...
       # else:
           ...
           #     time.sleep(5)
        print("initializing ... config", args.config)
        print("initializing ... checkpoint", checkpoint)
        model = init_detector(
